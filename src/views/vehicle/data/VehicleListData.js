...
export const useVehicleProfileData = () => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/vehicle/get/${id}`, {
        headers: {
...
export const postVehicleServiceApi = async (servicesystem, id) => {
  try {
    const response = await axios.post(
...
// Service Vehicle current odometer reading

export const getServerOdometerApi = async (id) => {
  if (!TOKEN) throw new Error('Authentication token not found')

  const { data } = await axios.get(
    `${import.meta.env.VITE_API_URL}/api/service/vehicle-odo-distance?vehicleId=${id}`,
    {
      headers: { Authorization: `Bearer ${TOKEN}` },
    },
  )

  console.log('Full API Response:', data) // Debug: Check what the API returns

  // Handle different response structures
  let odometerData = data.data || data

  // If it's an object with vehicleId property
  if (odometerData && typeof odometerData === 'object' && !Array.isArray(odometerData)) {
    return [
      {
        id: odometerData.vehicleId || id,
        totalKm: odometerData.totalKm || odometerData.currentOdometer || 0,
        cacheDate: odometerData.cacheDate || new Date().toISOString(),
      },
    ]
  }

  // If it's an array
  if (Array.isArray(odometerData)) {
    return odometerData.map((item) => ({
      id: item.vehicleId || id,
      totalKm: item.totalKm || item.currentOdometer || 0,
      cacheDate: item.cacheDate || new Date().toISOString(),
    }))
  }
