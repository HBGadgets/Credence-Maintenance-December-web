...
export const getConsigneeApi = async ({ queryKey }) => {
  ...
  const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/api/consignee/get`, {
    params: {
      search: search || '',
      page,
      limit,
...
export const deleteConsigneeApi = async (id) => {
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_API_URL}/api/consignee/softdelete/${id}`,
      {
        headers: { Authorization: `Bearer ${TOKEN}` },
      },
    )
...
export const getConsignorApi = async ({ queryKey }) => {
  ...
  const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/api/consignor/get`, {
    params: {
      search: search || '',
      page,
      limit,
...
export const deleteConsignorApi = async (id) => {
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_API_URL}/api/consignor/softdelete/${id}`,
      {
        headers: { Authorization: `Bearer ${TOKEN}` },
      },
    )
