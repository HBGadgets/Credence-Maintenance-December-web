...
const ApprovedLeaveTable = () => {
  const { id } = useParams()
  const { state } = useLocation()
  const attendanceLocData = state?.approvedData || []
  ...
  return (
    <>
      ...
      <div className="card shadow-sm border-0 mb-4">
        <div className="card-body d-flex justify-content-between align-items-center p-3">
          <h5 className="mb-0 fw-semibold text-dark">
            Approved Leave Attendance: <span className="fw-bold">{driverName}</span> | Month:{' '}
      ...
      {/* Attendance Table */}
      <Table
        title="Driver Approved Leave Records"
        columns={columns}
