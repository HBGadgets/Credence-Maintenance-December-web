...
const InvoiceForm = ({ addModalOpen, setAddModalOpen, editModalOpen,setEditModalOpen,handleModalClose, invoiceToEdit, setFilteredInvoices, fetchInvoices }) => {
  ...
  const addItem = () => {
    ...
    setInvoiceData({
      ...invoiceData,
      items: [...invoiceData.items, { name: '',description: '', unitPrice: '', quantity: '', total: '' }]
    });
  };
  ...
  const handleCreateSubmit = async (e) => {
    ...
    const subTotal = invoiceData.items.reduce((total, item) => 
      total + (parseFloat(item.unitPrice || 0) * parseFloat(item.quantity || 0)), 0);
...
<FormControl fullWidth variant="outlined" margin="normal">
 ...
 <Typography variant="h5" gutterBottom>Items</Typography>
      {invoiceData.items.map((item, index) => (
        <Box key={index} sx={{ mb: 2, p: 2, border: '1px solid #ddd', borderRadius: 2 }}>
          <Grid container spacing={2}>
            ...
            {/* Remove Item Button */}
            <Grid item xs={12}>
              <Button variant="outlined" color="error" onClick={() => removeItem(index)} fullWidth>
                Remove Item
              </Button>
            </Grid>
          </Grid>
      ...
      <div className='d-grid ' style={{ display: 'grid', gridTemplateColumns: "1fr 1fr" , gridGap:'0rem 2rem' }}>
