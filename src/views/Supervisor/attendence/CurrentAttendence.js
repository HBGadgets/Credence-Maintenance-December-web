...
const CurrentAttendence = () => {
  ...
  const handleMarkAttendance = async (id) => {
    ...
    try {
      ...
      setFilteredData(prevData => prevData.filter(driver => driver.id !== id && driver._id !== id));
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to mark attendance', { position: 'top-right' });
    }
  };
